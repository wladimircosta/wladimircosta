# Olá, sou Wladimir Costa

**Cloud Solutions Architect — AWS & Azure** | Pré-Vendas Técnicas

## Sobre mim

Mais de 16 anos de carreira em TI, com passagem por AWS e Azure. Nos últimos 6 anos, meu foco é AWS — arquitetando soluções para empresas de grande porte do setor bancário/financeiro, à frente de migrações, modernização e pré-vendas técnicas e comerciais. Fui responsável por parcerias AWS-Microsoft, implementando os programas MMP e OLA da AWS.

## Experiência em destaque

**Arquitetura Cloud AWS (setor bancário/financeiro, últimos 6 anos)**
Arquitetura de soluções em nuvem AWS, incluindo parcerias de migração Microsoft → AWS, projetos de transformação digital, FinOps e pré-vendas técnicas e comerciais, para empresas de grande porte do setor bancário/financeiro. Condução de treinamentos Immersion Day para clientes e equipes internas.

**Migrações e modernização multicloud/híbrida**
Implementação de soluções em nuvem AWS e Microsoft Azure, com planejamento, migração e modernização de ambientes corporativos — incluindo integração entre on-premises e cloud, e migração de sistemas legados (incluindo mainframe), sempre com foco em segurança, automação e desempenho.

## Stack

`AWS` · `Microsoft Azure` · `Terraform` · `Multicloud` · `Infraestrutura Híbrida` · `FinOps` · `Active Directory`

## Contato

[LinkedIn](https://www.linkedin.com/in/wladimir-costa)
